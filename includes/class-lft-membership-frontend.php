...
class LFT_Membership_Frontend
{
	...
	/**
	 * 事務局が受け取るメール（BCC・事務局To）の件名先頭に【LFT管理通知】を付与（既に付いていればそのまま）
	 *
	 * @param string $subject
	 * @return string
	 */
	public static function prefix_lft_admin_mail_subject($subject)
	{
		$subject = trim((string) $subject);
		if ('' === $subject) {
			return $subject;
		}
		if (strpos($subject, '【LFT管理通知】') === 0) {
			return $subject;
		}
	...
	/**
	 * 現在リクエストのパスを、サイトホームからの相対パス（先頭・末尾スラッシュなし）で取得
	 *
	 * @return string 例: lft_membership/login または空
	 */
	private function get_request_path_relative_to_home()
	{
		...
		if (strpos($request_path, $home_path) === 0) {
			$rel = substr($request_path, strlen($home_path));
			return trim($rel, '/');
		}
		return trim($request_path, '/');
	}
	...
	/**
	 * ブラウザのURLが /{lft_membership_slug}/ 配下か（ログイン画面などリライト含む）
	 *
	 * @return bool
	 */
	private function is_request_uri_lft_membership_scope()
	{
		$rel    = $this->get_request_path_relative_to_home();
		$prefix = $this->slug;
		if ('' === $rel) {
			return false;
		}
		$in = ($rel === $prefix) || (strpos($rel, $prefix . '/') === 0);
		return (bool) apply_filters('lft_membership_is_request_uri_scope', $in, $rel);
	}
	...
	/**
	 * パスワード保護ページで、LFT 会員ログイン中（有効会員）ならサイト全体でパスワード入力を不要にする
	 * 認証はプラグインの会員DB＋Cookie のみ使用（/lft_membership 配下に限らない）
	 *
	 * @param bool    $required パスワードが必要か
	 * @param WP_Post $post
	 */
	public function filter_post_password_required($required, $post)
	{
		...
		if (! apply_filters('lft_membership_member_access_any_password_post', true, $post, $member)) {
			return $required;
		}
		return false;
	}
...
?>
	...
	/**
	 * マイページ固定ボタンを出す画面か
	 * URL が /{lft_membership}/ 配下なら表示（ログイン・会員ページの親でパスワード未設定でも可）。
	 * 会員未ログイン時は get_current_lft_member() で弾かれるため、ここは「エリア判定」のみ。
	 *
	 * @return bool
	 */
	private function should_show_mypage_button()
	{
		...
		if ($this->is_request_uri_lft_membership_scope()) {
	...
	public function inject_logout_button_script()
	{
		...
		$confirm_message = __('【重要】最終確認メールを、変更前（現在）のメールアドレス宛に送信しました。24時間以内に承認URLをクリックすると変更が確定します。確定後、新しいメールアドレス宛に完了通知が届きます。', 'lft-membership');
...
MAIL;
	...
	public function handle_email_change_request_api()
	{
		...
		// 最終確認メールは「変更前（現在）のログインID」宛のみ（新アドレス宛は承認完了後に別文面で送信）
		...
		wp_send_json_success(array(
			'message' => '【重要】メールアドレス変更の最終確認メールを、変更前（現在）のメールアドレス宛に送信しました。24時間以内にメール内の承認URLをクリックすると、新しいメールアドレスへの変更が確定します。確定後、新しいメールアドレス宛に完了通知が届きます。',
		));
	}
	...
	public function handle_confirm_email_change_page()
	{
		...
		$this->notify_office_email_change_completed($member->user_name, $old_email, $new_email);
		$login_url = home_url('/' . $this->slug . '/login/');
...
MAIL;
	...
	/**
	 * メール変更の最終確認メールを送信（変更前のメールアドレス宛のみ）
	 *
	 * @param string $old_email   現在登録中のメール（ログインID）
	 * @param string $user_name   登録名
	 * @param string $new_email   変更予定の新メールアドレス
	 * @param string $confirm_url 承認URL
	 * @return bool
	 */
	private function send_email_change_confirmation_email($old_email, $user_name, $new_email, $confirm_url)
	{
		if (empty($old_email) || ! is_email($old_email) || empty($new_email) || ! is_email($new_email)) {
			return false;
		}
		...
		$subject = apply_filters(
			...
			'【重要】メールアドレス（ログインID）変更の最終確認',
			$old_email,
			$new_email,
			$confirm_url
		);
		$body = $this->get_email_change_confirmation_email_body($display, $old_email, $new_email, $confirm_url);
		$body = apply_filters(
		...
		return (bool) wp_mail($old_email, $subject, $body, self::get_plugin_mail_headers());
	}

	/**
	 * メール変更最終確認メール本文（変更前メールアドレス宛）
	 *
	 * @param string $user_name     登録名（宛名）
	 * @param string $current_email 変更前メールアドレス
	 * @param string $new_email     変更後メールアドレス
	 * @param string $confirm_url   承認URL
	 * @return string
	 */
	private function get_email_change_confirmation_email_body($user_name, $current_email, $new_email, $confirm_url)
	{
		$name = $user_name;
		return <<<MAIL
【{$name}】様
...
LFT事務局の根岸でございます。
マイページより、
ログインID（メールアドレス）の変更申請を受け付けました。

セキュリティ保護のため、まだ変更は完了しておりません。

以下の内容をご確認の上、
24時間以内に承認URLをクリックしてください。

URLをクリックした時点で、
新しいメールアドレスへの変更が確定いたします。
...
※メルマガ等の配信先について※
ログインIDの変更確定後、順次、事務局にて
「定期メルマガ」等の配信先アドレスも新しいものへ更新させていただきます。
反映まで数日お時間をいただく場合がございますが、あらかじめご了承ください。
※新しいIDが確定した後は、
　以前のIDではログインできなくなりますのでご注意ください。
※本メールに心当たりがない場合は、
　第三者による誤入力の可能性があります。
　その場合はURLをクリックせずに本メールを破棄していただくか、
　お手数ですが事務局までご連絡をお願いいたします。
...
MAIL;
	...
	private function send_email_changed_notice_email($email, $user_name)
	{
		...
		$login_url = trailingslashit(untrailingslashit(home_url('/' . $this->slug . '/login/')));
		$subject   = apply_filters(
		...
		$body = $this->get_email_changed_notice_email_body($display, $email, $login_url);
		$body = apply_filters(
	...
	/**
	 * メール変更完了通知本文（新ログインID宛）
	 *
	 * @param string $user_name   登録名（宛名）
	 * @param string $new_email   新しいログインID（メール）
	 * @param string $login_url   ログインページURL（末尾スラッシュ付き推奨）
	 * @return string
	 */
	private function get_email_changed_notice_email_body($user_name, $new_email, $login_url)
	{
		$name = $user_name;
		return <<<MAIL
【{$name}】様
...
LFT事務局の根岸でございます。
メールアドレス（ログインID）の変更手続きがすべて完了いたしました。

今後は、以下の新しいログインIDと、
現在お使いのパスワードにてログインいただけます。
...
※以前のメールアドレスではログインできなくなっておりますので、
　ブラウザ等に保存されているログイン情報を更新してご利用ください。

また、本システムの変更に伴い、事務局よりお送りしている
「定期メルマガ」等の配信先も、順次新しいアドレスへ更新させていただきます。
反映まで今しばらくお待ちください。
...
MAIL;
	...
	/**
	 * 事務局へ「メールアドレス変更完了」の管理通知（アスメル等の同期用）
	 *
	 * @param string $user_name 会員名
	 * @param string $old_email 変更前メールアドレス
	 * @param string $new_email 変更後メールアドレス
	 */
	private function notify_office_email_change_completed($user_name, $old_email, $new_email)
	{
		...
		$old_email = is_string($old_email) ? trim($old_email) : '';
		$new_email = is_string($new_email) ? trim($new_email) : '';
		if (! is_email($old_email) || ! is_email($new_email)) {
			return;
		}
		...
		wp_mail($to, $subject, $body, self::get_plugin_mail_headers());
	}

	/**
	 * 事務局向け「メールアドレス変更完了」本文
	 *
	 * @param string $display    会員名
	 * @param string $old_email
	 * @param string $new_email
	 * @param string $updated_at サイト時刻での更新日時表示
	 * @return string
	 */
	private function get_office_email_change_completed_body($display, $old_email, $new_email, $updated_at)
	{
		return <<<MAIL
LFT事務局 担当者様

会員システムより、以下の通り
登録情報（メールアドレス）の更新が完了しました。
...
MAIL;
	...
	private function notify_office_member_profile_updated($user_name, $member_email, $change_description, $bcc_seminar = true)
	{
		...
		$subject = self::prefix_lft_admin_mail_subject(
			sprintf(
				/* translators: %s: member display name */
